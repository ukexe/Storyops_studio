...
- **OpenAI is the disclosed live provider:** Production inference uses the
  Responses API with strict structured output, `store: false`, bounded inputs,
  and `openai/<model>` audit IDs. Never describe this output as Granite.
- **Provider secrets stay server-side:** `OPENAI_API_KEY` belongs only in the
  `storyops-api` Cloudflare Worker secret store—never the frontend Worker,
  source files, documentation, screenshots, or chat.
...
- **Canonical FastAPI environment:** `WATSONX_API_KEY`,
  `WATSONX_PROJECT_ID`, and `WATSONX_URL` are required for Granite. The live
  edge adapter uses `OPENAI_API_KEY` and falls back deterministically when the
